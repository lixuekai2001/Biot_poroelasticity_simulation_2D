function [x,y] = Nodes2D(N,alphain)
...
% Combine blend & warp
warp1 = blend1.*warpf1.*(1 + sign(alpha)*(alpha*L1).^2);
warp2 = blend2.*warpf2.*(1 + sign(alpha)*(alpha*L2).^2);
warp3 = blend3.*warpf3.*(1 + sign(alpha)*(alpha*L3).^2);
